...
"""
s_full.py - Full Reference Agent (OpenAI SDK Version)
...
TOKEN_THRESHOLD = 50000 # Lowered to avoid OpenAI api token limits quicker
POLL_INTERVAL = 5
IDLE_TIMEOUT = 60
...
def run_subagent(prompt: str, agent_type: str = "Explore") -> str:
    ...
    resp = None
    for _ in range(30):
        resp = get_client().chat.completions.create(model=MODEL, messages=sub_msgs, tools=sub_tools, max_tokens=8000)
        message = resp.choices[0].message
        
        if resp.choices[0].finish_reason != "tool_calls":
            sub_msgs.append({"role": "assistant", "content": message.content or ""})
            break
    ...
    if resp and hasattr(resp.choices[0].message, "content"):
...
if __name__ == "__main__":
    ...
    while True:
        try:
            query = input("\033[36ms_full >> \033[0m").strip()
